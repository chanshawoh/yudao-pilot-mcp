...
优先把 Yudao Pilot 安装为隔离的命令行工具：

```bash
uv tool install yudao-pilot-mcp
```

没有 `uv` 时，使用标准 Python 虚拟环境和 `pip`：
...
Windows 激活虚拟环境时使用 `.venv\Scripts\activate`。本项目是带独立命令入口的 stdio MCP Server，适合用 pipx 隔离安装：

```bash
pipx install yudao-pilot-mcp
```

如果当前配置的 PyPI 镜像尚未同步新版本，显式使用官方 PyPI：
...
该命令会以前台 stdio 模式启动 MCP Server；由 MCP 客户端启动时无需手工运行。
...
非 `uv` 环境如果 `yudao-pilot` 不在 MCP 客户端的 `PATH` 中，直接配置虚拟环境里的 Python：
...
Windows 将 `command` 改为虚拟环境中 `Scripts\\python.exe` 的绝对路径。

临时试用可让 MCP 客户端通过 pipx 自动创建缓存环境；长期使用优先执行一次 `pipx install`，避免首次启动等待下载：

```json
{
  "mcpServers": {"yudao-pilot": {
    "command": "pipx",
