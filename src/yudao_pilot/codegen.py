    ...
    *,
    ...
    if output_path.exists() and not overwrite:
        return {
            ...
            "written": False,
            "existing_logical_migration": True,
            "message": "同名逻辑迁移文件已存在，未重复生成",
        }
    ...
    return {
        ...
        "path": str(output_path),
        "written": True,
        "existing_logical_migration": bool(migration_plan.get("existing")),
        "message": "迁移文件已生成",
    }
...
def build_migration_plan(backend_root: Path, migration_name: str) -> dict[str, Any]:
    ...
    existing = find_existing_migration_file(migration_dir, migration_name)
    filename = existing.name if existing else generate_laravel_style_migration_filename(migration_name)
    return {
...
def find_existing_migration_file(migration_dir: Path, migration_name: str) -> Path | None:
    if not migration_dir.exists():
        return None
    suffix = f"_{normalize_snake_case(migration_name)}.sql"
    candidates = sorted(path for path in migration_dir.glob(f"*{suffix}") if path.is_file())
