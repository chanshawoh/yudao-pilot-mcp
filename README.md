...
如果配置文件不存在，MCP 服务会尝试在明确的工作区根目录下初始化配置。若 MCP 服务进程工作目录是文件系统根目录、传入的 `workspace_root` 指向文件系统根目录，或当前目录下没有识别到受支持的 yudao 后端/前端项目，服务会返回 `workspace_root_required`，要求 AI 应用先询问用户真实项目工作目录，再带上 `workspace_root` 重试，避免把配置写到系统根目录或其它错误位置。
